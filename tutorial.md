...
4. **内容一致性核查**：LLM NLI 判断引用声明是否忠于论文摘要
5. **分类决策**：输出 5 类判定 —— VERIFIED / FABRICATED / METADATA_ERROR / MISREPRESENTED / UNVERIFIABLE
6. **两种前端**：FastAPI + SSE 流式单页应用 + Gradio 备选界面
...
- HIGH/MEDIUM 视为命中
- 同分 tiebreak：`openalex > crossref > serper > serpapi > google_scholar`
