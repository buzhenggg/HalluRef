...
- **两段级联检索**：Academic APIs -> Google Search；其中 Google Search 优先走 Search API，未配置 API key 时才启用 Google Scholar Direct 爬虫
- **元数据比对**：基于 `rapidfuzz` 相似度算法校验标题、作者、年份、venue
- **五类判定标签**：`VERIFIED` · `VERIFIED_MINOR` · `FABRICATED` · `METADATA_ERROR` · `UNVERIFIABLE`
- **多种调用方式**：SSE 流式接口、Web 单页前端、命令行
...
```bash
git clone https://github.com/buzhenggg/HalluRef.git
cd HalluRef
